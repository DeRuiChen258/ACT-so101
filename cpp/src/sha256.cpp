...
class Sha256 {
 public:
  ...
  void update(const uint8_t* data, size_t len) {
    total_bytes_ += len;
    while (len > 0) {
      const size_t take = std::min(len, kBlock - buffer_len_);
      std::copy(data, data + take, buffer_.begin() + static_cast<long>(buffer_len_));
      buffer_len_ += take;
      data += take;
      len -= take;
      if (buffer_len_ == kBlock) {
  ...
  std::string final_hex() {
    const uint64_t bits = total_bytes_ * 8ULL;
 ...
 private:
  ...
  std::array<uint8_t, kBlock> buffer_{};
  ...
  void compress(const uint8_t* block) {
    ...
    for (int i = 0; i < 64; ++i) {
      ...
      const uint32_t t1 = h + s1 + ch + kK[static_cast<size_t>(i)] + w[i];
...
std::string sha256_file(const std::filesystem::path& path) {
  ...
  while (in) {
    ...
    if (got > 0) {
      hasher.update(reinterpret_cast<const uint8_t*>(buffer.data()), static_cast<size_t>(got));
    }
  }
  return hasher.final_hex();
}
